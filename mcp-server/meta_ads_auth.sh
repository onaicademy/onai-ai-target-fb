...
# Загружаем токен из .env (см. .env.example в корне репо)
if [ -f "../.env" ]; then export $(grep -v "^#" ../.env | xargs); fi
if [ -z "$PIPEBOARD_API_TOKEN" ]; then echo "Error: PIPEBOARD_API_TOKEN не задан. Добавь его в .env"; exit 1; fi
...
if [ $? -ne 0 ]; then
    ...
    echo "curl -X POST \"http://localhost:3000/api/meta/auth?api_token=${PIPEBOARD_API_TOKEN}\" -H \"Content-Type: application/json\""
    echo ""
...
AUTH_RESPONSE=$(curl -s -X POST "http://localhost:3000/api/meta/auth?api_token=${PIPEBOARD_API_TOKEN}" -H "Content-Type: application/json")
