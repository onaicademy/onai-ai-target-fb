...
async def create_ad(
    ...
    Args:
        ...
        status: Initial ad status (default: ACTIVE)
...
async def upload_ad_image(
    ...
    Args:
        ...
        file_path: Local file path to an image (e.g., "/path/to/image.png")
    ...
    # Ensure we have image data from one of the sources
    if not file and not image_url and not file_path:
        return json.dumps({"error": "Provide either 'file' (data URL or base64), 'image_url', or 'file_path'"}, indent=2)
    ...
    try:
        ...
        if file_path:
            ...
            if file.startswith(data_url_prefix) and base64_marker in file:
                ...
                image_bytes = base64.b64decode(base64_payload.strip())

                # Infer content type and name from MIME type
                ...
                image_bytes = base64.b64decode(file.strip())
                if not inferred_name:
        ...
        if not image_bytes or len(image_bytes) < 100:
            return json.dumps({
                ...
                "details": f"Image bytes length: {len(image_bytes) if image_bytes else 0}"
            }, indent=2)

        print(f"Uploading image to Facebook Ad Account {account_id} (size: {len(image_bytes)} bytes)")

        # Use multipart/form-data upload (same as agent-service implementation)
        # This is the correct way to upload images to Facebook Ads API
        url = f"https://graph.facebook.com/v22.0/{account_id}/adimages"

        # Save to temp file for multipart upload (like agent-service does)
        tmp_path = os.path.join(tempfile.gettempdir(), f"fb_image_{uuid.uuid4().hex}.jpg")
        try:
            with open(tmp_path, "wb") as f:
                f.write(image_bytes)

            async with httpx.AsyncClient(timeout=120.0) as client:
                # Use multipart form data with 'filename' field (same as n8n/agent-service)
                with open(tmp_path, "rb") as f:
                    files = {"filename": (final_name, f, content_type)}
...
async def create_ad_creative(
    ...
    Args:
        ...
        call_to_action_type: Call to action button type (e.g., 'LEARN_MORE', 'SIGN_UP', 'SHOP_NOW', 'WHATSAPP_MESSAGE')
        ...
        whatsapp_phone_number: WhatsApp phone number without + (e.g., '77077707066') for WHATSAPP_MESSAGE CTA
        client_question: Pre-filled message user sends to WhatsApp (for WHATSAPP_MESSAGE campaigns)
        url_tags: UTM parameters to append to destination URL (e.g., 'utm_source=facebook&utm_medium=cpc')
        instagram_username: Instagram username (without @) for auto-generating Instagram profile URL as link_url
    ...
    # Auto-generate Instagram profile URL from username if provided
    if instagram_username and not link_url:
        # Remove @ if user included it
        username = instagram_username.lstrip('@')
        link_url = f"https://www.instagram.com/{username}/"
    ...
    if headlines or descriptions:
        ...
        if call_to_action_type:
            ...
            # Add link to value for website/traffic campaigns (SIGN_UP, SHOP_NOW, LEARN_MORE etc)
            elif link_url and call_to_action_type not in ["WHATSAPP_MESSAGE"]:
                cta_data["value"] = {"link": link_url}

            creative_data["object_story_spec"]["link_data"]["call_to_action"] = cta_data

        # Add WhatsApp page_welcome_message for WHATSAPP_MESSAGE campaigns
        if call_to_action_type == "WHATSAPP_MESSAGE" and client_question:
            # Build page_welcome_message with VISUAL_EDITOR format (same as video creatives)
            ...
            creative_data["object_story_spec"]["link_data"]["page_welcome_message"] = page_welcome_message
    
    # Add dynamic creative spec if provided
    if dynamic_creative_spec:
        creative_data["dynamic_creative_spec"] = dynamic_creative_spec

    # Add Instagram user ID to object_story_spec for Instagram placements
    # Must be inside object_story_spec, not at creative level
...
async def create_video_creative(
    ...
    """
    Create a video ad creative for WhatsApp, Instagram, Website, or Lead Form campaigns.

    This function creates video creatives using object_story_spec with video_data structure.
    It supports multiple campaign types based on call_to_action_type:

    - WHATSAPP_MESSAGE: Click-to-WhatsApp campaigns (requires whatsapp_phone_number, client_question)
    - LEARN_MORE: Instagram/Website traffic or Lead Form campaigns
    - SIGN_UP: Website conversion campaigns

    Args:
        account_id: Meta Ads account ID (format: act_XXXXXXXXX)
        video_id: ID of the uploaded video (from upload_video)
        page_id: Facebook Page ID to be used for the ad
        access_token: Meta API access token (optional - will use cached token if not provided)
        name: Creative name (auto-generated if not provided)
        message: Ad copy/text shown above the video
        call_to_action_type: CTA button type. Common values:
            - 'WHATSAPP_MESSAGE' for Click-to-WhatsApp
            - 'LEARN_MORE' for traffic/lead gen campaigns
            - 'SIGN_UP' for website conversions
        link_url: Destination URL (for LEARN_MORE/SIGN_UP campaigns)
        whatsapp_phone_number: WhatsApp number without + (e.g., '77077707066') for WHATSAPP_MESSAGE
        client_question: Pre-filled message user sends to WhatsApp (for WHATSAPP_MESSAGE)
        lead_gen_form_id: Lead form ID for lead generation campaigns
        instagram_actor_id: Instagram account ID for Instagram placements
        thumbnail_hash: Optional image hash for video thumbnail
        thumbnail_url: Optional image URL for video thumbnail (auto-fetched from video if not provided)
        url_tags: UTM parameters to append to destination URL (e.g., 'utm_source=facebook&utm_medium=cpc')
        instagram_username: Instagram username (without @) for auto-generating Instagram profile URL as link_url

    Returns:
        JSON response with created creative ID and details
    ...
    Example (WhatsApp):
        create_video_creative(
            ...
            video_id="987654321",
            page_id="123456789",
            message="Напишите нам, чтобы узнать подробности",
    ...
    if not video_id:
        return json.dumps({"error": "No video_id provided"}, indent=2)

    if not page_id:
        return json.dumps({"error": "No page_id provided"}, indent=2)

    if not name:
        name = f"Video Creative {int(time.time())}"
    ...
    # If no thumbnail provided, fetch from video
    if not thumbnail_hash and not thumbnail_url:
        try:
            video_data_response = await make_api_request(
                endpoint=video_id,
                access_token=access_token,
                params={"fields": "thumbnails"},
                method="GET"
            )
            thumbnails = video_data_response.get("thumbnails", {}).get("data", [])
            if thumbnails:
                # Find preferred thumbnail or use first one
                preferred = next((t for t in thumbnails if t.get("is_preferred")), thumbnails[0])
                thumbnail_url = preferred.get("uri")
        except Exception as e:
            # Continue without thumbnail - will fail at API call
            pass

    # Build video_data structure
    ...
    # Add thumbnail - prefer hash over URL
    if thumbnail_hash:
        video_data["image_hash"] = thumbnail_hash
    elif thumbnail_url:
        video_data["image_url"] = thumbnail_url

    # Build call_to_action based on type
    if call_to_action_type:
        cta: Dict[str, Any] = {"type": call_to_action_type}

        if call_to_action_type == "WHATSAPP_MESSAGE":
            # WhatsApp CTA with optional phone number
            if whatsapp_phone_number:
                cta["value"] = {"whatsapp_number": whatsapp_phone_number}

            # Add page_welcome_message for WhatsApp autofill
            if client_question:
                ...
                video_data["page_welcome_message"] = json.dumps(page_welcome_message)
